<template>
        ...
        >
          <div>
            <a-form ref="myform" :label-col="{ style: { width: '120px' } }" :model="modal.form" :rules="modal.rules">
              <a-row :gutter="24">
                <a-col span="24">
                  <a-form-item label="Term name" name="title">
                    <a-input placeholder="Please enter" v-model:value="modal.form.title"></a-input>
                  </a-form-item>
                </a-col>
                <a-col span="24">
                  <a-form-item label="Start Date" name="expect_time">
                    <input type="date" v-model="modal.form.expect_time">
                  </a-form-item>
                </a-col>
                <a-col span="24">
                  <a-form-item label="End Date" name="return_time">
                    <input type="date" v-model="modal.form.return_time">
                  </a-form-item>
                </a-col>
              </a-row>
            </a-form>
  ...
  <script setup lang="ts">
    ...
    import { createApi, listApi, updateApi, deleteApi } from '/@/api/admin/term';
    ...
    const modal = reactive({
      ...
      rules: {
        ...
        expect_time: [{ required: true, message: 'Please choose', trigger: 'change' }],
        return_time: [{ required: true, message: 'Please choose', trigger: 'change' }],
      },
    });
      ...
      listApi({
        ...
        .then((res) => {
          ...
          res.data.forEach((item: any, index: any) => {
            ...
            //format yyyy-mm-dd
            item.expect_time = item.expect_time ? item.expect_time.split("T")[0] : '';
            item.return_time = item.return_time ? item.return_time.split("T")[0] : '';
